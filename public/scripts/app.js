...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__utils_data__ = __webpack_require__(2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__core_Element__ = __webpack_require__(6);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__core_Search__ = __webpack_require__(7);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__core_Modal__ = __webpack_require__(8);
...
var el = {
   ...
   */
  offsetList: function offsetList() {
    return this.searchList.getBoundingClientRect().top;
  }
};

function runApp(data) {
  var items = Object(__WEBPACK_IMPORTED_MODULE_0__utils_data__["b" /* getItems */])(data);
  var element = new __WEBPACK_IMPORTED_MODULE_1__core_Element__["a" /* default */](items);
  ...
  ['input', 'keyup'].forEach(function (event) {
    el.searchInput.addEventListener(event, function (e) {
      ...
      var search = new __WEBPACK_IMPORTED_MODULE_2__core_Search__["a" /* default */](this.value.toLowerCase());

      [].forEach.call(el.categories, function (cat) {
        search.filterCategories(cat);
      });

      [].forEach.call(el.genres, function (genre) {
        search.filterGenres(genre);
      });

      //reset menu filters
      [].forEach.call(el.menuItems, function (item) {
  ...
  [].forEach.call(el.links, function (link) {
    link.addEventListener('click', function (e) {
      e.preventDefault();
      window.open(this.getAttribute('href'), this.getAttribute('title'), "resizable,scrollbars=yes,status=1");
    });
  });

  // Create the Filters Menu Modal
  var modal = new __WEBPACK_IMPORTED_MODULE_3__core_Modal__["a" /* default */](el.aside, el.offCanvas, el.modalTitle, el.modalContent);

  // Toggle the filters Menu on click
  [].forEach.call(el.btnModal, function (btn) {
  ...
  [].forEach.call(el.menuItems, function (item) {
    item.addEventListener('click', function (e) {
      ...
      var search = new __WEBPACK_IMPORTED_MODULE_2__core_Search__["a" /* default */]();
      search.reset(el.categories);

      el.searchInput.value = val;

      var menuSearch = new __WEBPACK_IMPORTED_MODULE_2__core_Search__["a" /* default */](val);
  ...
  el.btnCallToAction.addEventListener('click', function (e) {
    e.preventDefault();
    window.scroll(0, 0);
 ...
 */
Object(__WEBPACK_IMPORTED_MODULE_0__utils_data__["a" /* fetchData */])(url).then(function (data) {
  return runApp(data);
}).catch(function (err) {
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 8:
            ...
            console.error("Failed to retrieve data: (" + _context.t0 + ")");
...
var Element = function () {
  ...
  _createClass(Element, [{
    key: 'renderCategory',
    value: function renderCategory(cat) {
    ...
    key: 'renderMenuList',
    value: function renderMenuList(genre) {
    ...
    key: 'renderMenuItemAll',
    value: function renderMenuItemAll() {
...
var Search = function () {
  ...
  _createClass(Search, [{
    ...
    value: function highlightItem(item) {
      var arr = item.getAttribute('title').toLowerCase().split(this.val);

      return arr.join('<span class="is-highlight" tabindex="-1">' + this.val + '</span>');
    }
    ...
    value: function filterCategories(cat) {
      var title = cat.getAttribute('title').toLowerCase();
      if (!title.includes(this.val)) {
        ...
        var link = cat.querySelector('.search__category-link');
    ...
    key: 'filterGenres',
    value: function filterGenres(genre) {
      var childNodes = genre.querySelectorAll('.search__category-list > .search__category-item');
      var hiddenChildNodes = genre.querySelectorAll('.search__category-list > .search__category-item.is--hidden');

      var genreTitle = genre.querySelector('.search__genre-title');
      ...
      if (genreTitle.textContent.toLowerCase().includes(this.val)) {
        [].forEach.call(hiddenChildNodes, function (node) {
    ...
    key: 'reset',
    value: function reset(categories) {
      var _this = this;
      ...
      [].forEach.call(categories, function (cat) {
        ...
        cat.classList.remove('is--hidden');
        var link = cat.querySelector('.search__category-link');
        link.innerHTML = _this.highlightItem(cat);
      });
    }
    ...
    key: 'filterMenuGenres',
    value: function filterMenuGenres(genre) {
      if (this.val !== 'all') {
        genre.style.display = 'none';
...
var Modal = function () {
  ...
  _createClass(Modal, [{
    key: 'animateOnShow',
    value: function animateOnShow(offset) {
      [].forEach.call(this.others, function (other) {
        setTimeout(function () {
    ...
    key: 'animateOnHide',
    value: function animateOnHide(offset) {
      [].forEach.call(this.others, function (other) {
